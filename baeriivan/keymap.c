...
#define OS_LINUX_COLOR HSV_RED
#define OS_WINDOWS_COLOR HSV_GREEN

static uint8_t unicode_mode = UNICODE_MODE_LINUX; //default

void unicode_input_mode_set_user(uint8_t mode) {
    unicode_mode = mode;
}
    ...
    switch (get_highest_layer(state)) {
        case _GAMING:
            ...
            if (unicode_mode == UNICODE_MODE_LINUX) {
                rgblight_sethsv_noeeprom(OS_LINUX_COLOR);
            } else {
                rgblight_sethsv_noeeprom(OS_WINDOWS_COLOR);
            }
            break;
        case _BASE:
        default:
